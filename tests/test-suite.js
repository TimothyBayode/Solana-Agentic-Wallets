...
const AgentWallet_1 = require("../src/wallet/AgentWallet");
...
const w1 = AgentWallet_1.AgentWallet.create({ agentId: "test-agent-1" });
...
const w2 = AgentWallet_1.AgentWallet.create({ agentId: "test-agent-2" });
assert(w1.publicKey !== w2.publicKey, "Two agents have different keypairs");
suite("Wallet Persistence & Reload");
const reloaded = AgentWallet_1.AgentWallet.load("test-agent-1", w1.getEncryptionKeyHex());
assert(reloaded.publicKey === w1.publicKey, "Reloaded wallet matches original");
let threw = false;
try {
    AgentWallet_1.AgentWallet.load("test-agent-1", "0".repeat(64));
}
catch {
    threw = true;
}
...
const tx = new web3_js_1.Transaction().add(web3_js_1.SystemProgram.transfer({
    ...
    toPubkey: new web3_js_1.PublicKey(mockAddr),
